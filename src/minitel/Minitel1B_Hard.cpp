...
void Minitel::writeCode(unsigned long code)
{
    ...
    if (code > 0x00FFFFFF)
        writeByte((byte)(code >> 24));
    if (code > 0x0000FFFF)
        writeByte((byte)(code >> 16));
    if (code > 0x000000FF)
        writeByte((byte)(code >> 8));
    writeByte((byte)code);
}
/*--------------------------------------------------------------------*/
...
byte Minitel::readByte()
{
    ...
    if (bitRead(b,7) == parite) {  // La transmission est bonne, on peut récupérer la donnée.
      if (bitRead(b,7) == 1) {  // Cas où le bit de parité vaut 1.
        b = b ^ 0b10000000;  // OU exclusif pour mettre le bit de parité à 0 afin de récupérer la
    donnée.
      }
      return b;
    }
...
unsigned long Minitel::identifyDevice()
{ // Voir p.139
    // Fonction proposée par iodeo sur GitHub en février 2023
    // Demande
...
int Minitel::changeSpeed(int bauds)
{ // Voir p.141
    ...
    case 9600:
        writeByte(0b1111111);
        break; // 0x7F (pour le Minitel 2 seulement)
    }
#if defined(ESP32) || defined(ARDUINO_ARCH_ESP32)
...
void Minitel::cursor() { writeByte(CON); }
/*--------------------------------------------------------------------*/

void Minitel::noCursor() { writeByte(COFF); }
/*--------------------------------------------------------------------*/
...
int Minitel::getCursorX() { return (getCursorXY() & 0x0000FF) - 0x40; }
/*--------------------------------------------------------------------*/

int Minitel::getCursorY() { return ((getCursorXY() & 0x00FF00) >> 8) - 0x40; }
/*--------------------------------------------------------------------*/
...
void Minitel::clearScreenFromCursor()
{                   // Voir p.95
    writeWord(CSI); // 0x1B 0x5B
...
void Minitel::clearScreenToCursor()
{                   // Voir p.95
    writeWord(CSI); // 0x1B 0x5B
    writeByte(0x31);
    writeByte(0x4A);
...
void Minitel::clearLineFromCursor()
{                   // Voir p.95
    writeWord(CSI); // 0x1B 0x5B
    // writeByte(0x30);  Inutile
    writeByte(0x4B);
...
void Minitel::clearLineToCursor()
{                   // Voir p.95
    writeWord(CSI); // 0x1B 0x5B
    writeByte(0x31);
    writeByte(0x4B);
...
void Minitel::moveCursorG1(int x, int y)
{
    // CSI positioning does NOT reset G0/G1, unlike US/RS via newXY().
    // Re-assert SO after moving to guarantee mosaic.
    moveCursorXY(x, y);
    writeByte(SO);
}
/*--------------------------------------------------------------------*/
...
byte Minitel::standardTeleinformatique()
{ // Voir p.144
    ...
    */
    ...
    while (i < chaine.length()) {
        ...
        if (code < SP)
            ...
            case 0xC380:
                code = 0x0F41;
                break; // À (Aucune lettre accentuée majuscule n'est disponible - voir p.90)
            case 0xC382:
                code = 0x0F41;
                break; // Â (Aucune lettre accentuée majuscule n'est disponible - voir p.90)
            case 0xC384:
                code = 0x0F41;
                break; // Ä (Aucune lettre accentuée majuscule n'est disponible - voir p.90)
            case 0xC387:
                code = 0x0F43;
                break; // Ç (Aucune lettre accentuée majuscule n'est disponible - voir p.90)
            case 0xC388:
                code = 0x0F45;
                break; // È (Aucune lettre accentuée majuscule n'est disponible - voir p.90)
            case 0xC389:
                code = 0x0F45;
                break; // É (Aucune lettre accentuée majuscule n'est disponible - voir p.90)
            case 0xC38A:
                code = 0x0F45;
                break; // Ê (Aucune lettre accentuée majuscule n'est disponible - voir p.90)
            case 0xC38B:
                code = 0x0F45;
                break; // Ë (Aucune lettre accentuée majuscule n'est disponible - voir p.90)
            case 0xC38E:
                code = 0x0F49;
                break; // Î (Aucune lettre accentuée majuscule n'est disponible - voir p.90)
            case 0xC38F:
                code = 0x0F49;
                break; // Ï (Aucune lettre accentuée majuscule n'est disponible - voir p.90)
            case 0xC394:
                code = 0x0F4F;
                break; // Ô (Aucune lettre accentuée majuscule n'est disponible - voir p.90)
            case 0xC396:
                code = 0x0F4F;
                break; // Ö (Aucune lettre accentuée majuscule n'est disponible - voir p.90)
            case 0xC399:
                code = 0x0F55;
                break; // Ù (Aucune lettre accentuée majuscule n'est disponible - voir p.90)
            case 0xC39B:
                code = 0x0F55;
                break; // Û (Aucune lettre accentuée majuscule n'est disponible - voir p.90)
            case 0xC39C:
                code = 0x0F55;
                break; // Ü (Aucune lettre accentuée majuscule n'est disponible - voir p.90)
            case 0xC3A0:
                code = 0x194161;
                break; // à (VGP5 et VGP2)
            case 0xC3A2:
                code = 0x194361;
                break; // â (VGP5 et VGP2)
            case 0xC3A4:
                code = 0x194861;
                break; // ä (VGP5 seulement)
            case 0xC3A7:
                code = 0x194B63;
                break; // ç (VGP5 et VGP2)
            case 0xC3A8:
                code = 0x194165;
                break; // è (VGP5 et VGP2)
            case 0xC3A9:
                code = 0x194265;
                break; // é (VGP5 et VGP2)
            case 0xC3AA:
                code = 0x194365;
                break; // ê (VGP5 et VGP2)
            case 0xC3AB:
                code = 0x194865;
                break; // ë (VGP5 et VGP2)
            case 0xC3AE:
                code = 0x194369;
                break; // î (VGP5 et VGP2)
            case 0xC3AF:
                code = 0x194869;
                break; // ï (VGP5 et VGP2)
            case 0xC3B4:
                code = 0x19436F;
                break; // ô (VGP5 et VGP2)
            case 0xC3B6:
                code = 0x19486F;
                break; // ö (VGP5 seulement)
            case 0xC3B7:
                code = 0x1938;
                break; // ÷ (VGP5 et VGP2)
            case 0xC3B9:
                code = 0x194175;
                break; // ù (VGP5 et VGP2)
            case 0xC3BB:
                code = 0x194375;
                break; // û (VGP5 et VGP2)
            case 0xC3BC:
                code = 0x194875;
                break; // ü (VGP5 seulement)
            case 0xC592:
                code = 0x196A;
                break; // Œ (VGP5 et VGP2)
            case 0xC593:
                code = 0x197A;
                break; // œ (VGP5 et VGP2)
            case 0xCEB2:
                code = 0x197B;
                break; // β (VGP5 seulement)
            default:
                code = 0; // supposé non-visualisable
            }
        } else if (code == 0xE2) {
...
byte Minitel::getCharByte(char caractere)
{
    ...
    String caracteres =
        ...
        "!\"#$%&'()*+,-./"
        "0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_xabcdefghijklmnopqrstuvwxyz{|}";
...
{
    // Cette fonction est à utiliser en association avec getString(unsigned long code) juste
    // ci-dessus Elle renvoie le nombre d'octets d'un caractère codé en String UTF-8
    int nbBytes = 0;
    if (isVisualisable(code)) {
        if (code < 0x80) {           // U+0000 à U+007F
            nbBytes = 1;             // 1 octet
...
void Minitel::graphic(byte b)
{
    ...
    if (b <= 0b111111) {
        b = 0x20 + bitRead(b, 5) + bitRead(b, 4) * 2 + bitRead(b, 3) * 4 + bitRead(b, 2) * 8 +
            bitRead(b, 1) * 16 + bitRead(b, 0) * 64;
        if (b == 0x7F) { // 0b1111111
            b = 0x5F;
        }
        writeByte(b);
...
unsigned long Minitel::getKeyCode(bool unicode)
{
    ...
    if (code == 0x19) { // SS2
        ...
        if ((code == 0x1941) || (code == 0x1942) || (code == 0x1943) || (code == 0x1948) ||
            (code == 0x194B)) { // Accents, tréma, cédille
            // Bug 1 : Pour éviter de compter un caractère lorsqu'on appuie plusieurs fois de suite
            ...
            // Bug 2 : Pour éviter de compter un caractère lorsqu'on appuie sur les touches de
            // fonction après avoir appuyé sur une touche avec accent ou tréma
            if (caractere ==
...
byte Minitel::echo(boolean commande)
{ // Voir p.81, p.135 et p.156
    // Fonction modifiée par iodeo sur GitHub en octobre 2021
...
byte Minitel::aiguillage(boolean commande, byte emetteur, byte recepteur)
{ // Voir p.135
    // commande peut prendre comme valeur :
    // true, false
...
byte Minitel::statusAiguillage(byte module)
{ // Voir p. 136
...
byte Minitel::connexion(boolean commande)
{ // Voir p.139
...
boolean Minitel::isVisualisable(unsigned long code)
{
    ...
    case 0x00A3:
        return true; // £
    case 0x00A7:
        return true; // § (VGP5 seulement, pas VGP2 - voir p.90)
    case 0x00B0:
        return true; // ° Ctrl 0 au clavier
    case 0x00B1:
        return true; // ± Ctrl * au clavier
    case 0x00BC:
        return true; // ¼
    case 0x00BD:
        return true; // ½
    ...
    */
    ...
    case 0x00E2:
        return true; // â
    case 0x00E4:
        return true; // ä (VGP5 seulement, pas VGP2 - voir p.90)
    case 0x00E7:
        return true; // ç
    case 0x00E8:
        return true; // è
    ...
    case 0x00F4:
        return true; // ô
    case 0x00F6:
        return true; // ö (VGP5 seulement, pas VGP2 - voir p.90)
    case 0x00F7:
        return true; // ÷ Ctrl 7 au clavier
    case 0x00F9:
        return true; // ù
    case 0x00FB:
        return true; // û
    case 0x00FC:
        return true; // ü (VGP5 seulement, pas VGP2 - voir p.90)
    case 0x0152:
        return true; // Œ
    case 0x0153:
        return true; // œ
    case 0x03B2:
        return true; // β (VGP5 seulement, pas VGP2 - voir p.90)
    case 0x2014:
        return true; // — Ctrl 5 au clavier
    case 0x2190:
        return true; // ← Ctrl 8 au clavier
    case 0x2191:
        return true; // ↑
    case 0x2192:
        return true; // → Ctrl 9 au clavier
    case 0x2193:
        return true; // ↓ Ctrl # au clavier
    }
...
void Minitel::writeBytesPRO(int n)
{                   // Voir p.134
    writeByte(ESC); // 0x1B
    switch (n) {
    case 1:
...
int Minitel::workingSpeed()
{
    ...
    case 0x1B3A757F:
        bauds = 9600;
        break; // Pour le Minitel 2 seulement
    }
    return bauds;
}
/*--------------------------------------------------------------------*/
...
{   // Voir p.143
	...
	printf("status ok: %08lX\n", trame);
    return (byte)trame;
}
/*--------------------------------------------------------------------*/

byte Minitel::workingKeyboard()
{ // Voir p.142
...
byte Minitel::workingAiguillage(byte module)
{ // Voir p.136
...
unsigned long Minitel::getCursorXY()
{ // Voir p.98
    // Demande
    writeByte(ESC);
    writeByte(0x61);
...
bool Minitel::displayPng(const std::string& path, int threshold)
{
	const int maxCellsW = 40;
	const int maxCellsH = 24;
    ...
    // Use CSI positioning (moveCursorXY) which preserves G1. Single SO/SI
    // via RAII guard; no need to re-assert per row (unlike newXY/US which
    // resets to G0).
    {
        G1Guard g1(*this);
        for (int cy = 0; cy < cellsH; cy++) {
            moveCursorXY(startX, startY + cy);
