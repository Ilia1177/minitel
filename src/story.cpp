...
Minitel::State Minitel::add_contact(State endState, const std::string& input) {
	
	if (input.empty()) {
		// if no input, we display the Page and return the current state
		update_cursor(5, 19, 0);
		write_text(" Please provide your email address:\r\n");
		return State::EMAIL;
	} else {
		// else get the input and do stuff with it return the endState
		_contacts.open("contacts.txt", std::ios::app);
		if (!_contacts.is_open()) {
			std::cout << "Create contacts.txt:\n";
			std::ofstream create("contacts.txt");
			create.close();
			_contacts.open("story.txt", std::ios::app);
		}
		if (!_contacts.is_open()) {
...
Minitel::State Minitel::redirect_to(State state, bool waiting) {
	...
	switch (state) {
		case Minitel::State::MENU:
			return index_page(_state);
		case Minitel::State::HAZARDOUS:
			return hazardous_collective(_state);
		case Minitel::State::STORY:
			return cadavre_exquis(_state);
		case Minitel::State::EMAIL:
			return add_contact(_state);
		case Minitel::State::FORTY2:
			return forty_two(_state);
		default:
