...
public:
  ...
  // Uses CSI positioning (moveCursorXY) so a single SO/SI is sufficient.
  ...
  // Helper that positions and ensures G1 is active (re-asserts SO if needed).
  void moveCursorG1(int x, int y);
...
private: 
  ...
  struct G1Guard {
    Minitel &m;
    explicit G1Guard(Minitel &mm) : m(mm) { m.graphicMode(); }
    ~G1Guard() { m.textMode(); }
    G1Guard(const G1Guard&) = delete;
    G1Guard& operator=(const G1Guard&) = delete;
  };
};
